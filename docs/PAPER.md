...
**Suite and size.** The suite runs **176 tests** (unit, property, and end-to-end) green, hermetically (hash embedder and isolated state, no downloads). The engine is ~3,800 lines of Python; the tests, ~2,200. The variety gate (including its within-niche value assertion), the advisory originality probe, and the collapse reversal all pass, offline and `--live`.
