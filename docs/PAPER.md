...
```mermaid
flowchart TD
    A[1. Resolve session axes<br/>named / inferred+confirmed / generic] --> B[2. Generate candidates<br/>LLM agent · operator bank<br/>anti-cliché: steer away from the obvious-set]
...
**The narrow value assertion and the advisory probe.** The within-niche value check passes: with an identical descriptor the `fitness = 1.2` candidate becomes the niche elite and the `0.9` one does not, and swapping the two values flips the elite — confirming the real elite rule, not candidate order. The advisory originality probe (reported, not gated) puts the diverse slate at **0.766** distance-to-obvious against the held-out cliché half versus **0.646** for a deliberately clichéd slate — the engine's slate sits measurably further from the obvious answers, as it should.
...
**Suite and size.** The suite runs **203 tests** (unit, property, and end-to-end) green, hermetically (hash embedder and isolated state, no downloads). The engine is ~4,100 lines of Python; the tests, ~2,550. The variety gate (including its within-niche value assertion), the advisory originality probe, and the collapse reversal all pass, offline and `--live`.
