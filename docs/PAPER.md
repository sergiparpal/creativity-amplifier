...
> **Diversity is owned by geometry; the judge owns only validity.** Geometry (the engine) owns *novelty*: embeddings, MAP-Elites niching, k-NN novelty, DPP selection. The LLM (the agent) owns only *validity* and *within-niche ranking*: it filters what is off-brief and may assign a `fitness` ∈ [0,1] that ranks *within* a niche —it never prunes variety nor picks the final slate. The human is the real selector.
